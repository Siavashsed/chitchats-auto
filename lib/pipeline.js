...
async function intake(order, settings) {
  const pkg = computePackage(order, settings.packaging);
  order.pkg = pkg;

  const rec = store.upsertOrder({
    ...
    status: 'received',
    mode: settings.mode,
    events: [{ at: new Date().toISOString(), message: `Order received - ${pkg.summary} -> ${pkg.boxName} (${pkg.weight}g)` }],
  });
